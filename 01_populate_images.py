...
SQL_LOOKUP_IMAGE = "SELECT imgid, occid FROM images where originalurl = %s"


def url_lookup(mysql_config, mongo_url, collid, img):
...
def main():
    ...
    for doc in mongo_collection_collections.find():
        ...
        if img_res.ok:
            ...
            # SQL lookup of up to cpu_count imgid's at once
            with mp.Pool() as p:
                p.starmap(url_lookup, [(mysql_config, mongo_url, collid, img) for img in img_res_obj["items"]])
                p.join()
        else:
